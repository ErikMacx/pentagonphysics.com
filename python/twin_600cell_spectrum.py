...
described in McLean (2026), "The Other Side of Gravity from the 600-Cell Laplacian",
Section 8 and Appendix A.

Requires: numpy. Standard library otherwise.
Run time: approximately 1 second on a standard laptop.

Output: the 23 distinct eigenvalues with their multiplicities, totalling 216.
"""
...
# Identify the 24 D_4 bridge vertices (the inscribed 24-cell: types A and B).
...
# Add neutron edges (bridge-to-bridge edges already added above; they are identical).
for i in range(N):
    for j in range(N):
        if adj[i, j]:
...
print()
print(f"Spectrum: {len(groups)} distinct eigenvalues, total multiplicity {sum(len(g) for g in groups)}")
print()
print(f"  No.  Eigenvalue           Multiplicity")
print(f"  " + "-" * 50)
for i, g in enumerate(groups, 1):
    val = sum(g) / len(g)
    print(f"  {i:>3}  {val:>14.10f}     {len(g):>3}")

n_int = sum(len(g) for g in groups if abs(sum(g)/len(g) - round(sum(g)/len(g))) < 1e-4)
print()
